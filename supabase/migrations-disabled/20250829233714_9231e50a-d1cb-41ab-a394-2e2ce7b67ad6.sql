-- Drop existing storage policies properly
DROP POLICY IF EXISTS "Allow all storage operations for authenticated users" ON storage.objects;
DROP POLICY IF EXISTS "Public read access to club assets" ON storage.objects;
DROP POLICY IF EXISTS "Club admins can delete their assets" ON storage.objects;
DROP POLICY IF EXISTS "Club admins can update their assets" ON storage.objects;
DROP POLICY IF EXISTS "Club admins can upload assets" ON storage.objects;
DROP POLICY IF EXISTS "Club assets are publicly accessible" ON storage.objects;
...
-- Apply permissive policies to remaining tables
-- Club Members table
