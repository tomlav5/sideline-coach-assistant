...
- **Two live Supabase projects.** Production is `crmlmnhillnnrnrxqera` (`coach-asst`) — the real match data, what Vercel Production points at. Staging is `xszbopufqchbfbqwvqbb` (`sideline-staging`, West EU/London, created 30 Aug 2026) — what Vercel Preview/Development and all schema testing point at. `supabase/config.toml`'s `project_id` is the **production** ref, so any `supabase link` / `supabase db …` that accepts the default lands on the live database (ENV-008). Always pass `--project-ref` explicitly: `xszbopufqchbfbqwvqbb` for staging, `crmlmnhillnnrnrxqera` for production. A third project, `sideline_app` (`pngbyspcczhzkccjxupo`), is a redundant first attempt to be retired after the season (DEBT-023) — do not use it.
- Migrations live in `supabase/migrations/` (applied). As of the August 2026 baseline reset, this contains a single file, `20260824000408_baseline.sql` — the full live schema, declared as the new starting point rather than a replayable-from-scratch history (see `docs/SCHEMA_BASELINE.md` for why the old history couldn't be reconciled with the remote). New migrations must use full 14-digit `YYYYMMDDHHMMSS_name.sql` timestamps, not 8-digit dates — Supabase's CLI keys migration history off that prefix, and reused/short prefixes caused version collisions in the old history. Apply new migrations with `supabase db push` or via the Supabase dashboard SQL editor. `supabase/migrations-archive/` holds every migration that predates the baseline (`pre-baseline/` and `lovable-era/`) — kept for reference only and never run; see its `README.md`.
